...
main_page = getURL("https://www.weinfreunde.de/FLSearchAPI/search?sSearch=&order=none&count=36")
...
data = data.frame(Name=character(), Farbe=character(), Herkunftsland=character(), Herkunftsregion=character(),
                  ...
                  Jahrgang=integer(), Preis = numeric(), stringsAsFactors = FALSE)

# loop to scrape wine data 

for (i in 1:pages){
  main_page = getURL(paste("https://www.weinfreunde.de/FLSearchAPI/search?sSearch=&order=none&count=36&view=",i, sep = ""))
  ...
  for (j in 1:objects) {
    ...
    #check for wrong adresses
    test = try(htmlTreeParse(xpath_sub_page), silent = TRUE)
    if (isTRUE(class(test) == "try-error")){
      break
    }
    ...
    price = xpathApply(sub_doc, "//meta[@property='product:price']/@content")
    ...
    data_new = list(name, colour, origin_country, origin_region, grape, taste, style, quality, alcohol, 
                    temp, sweet, sour, ripe, expansion, producer, year, price)
...
data$Farbe = as.factor(data$Farbe)
data$Herkunftsland = as.factor(data$Herkunftsland)
data$Herkunftsregion = as.factor(data$Herkunftsregion)
data$Rebsorte = as.factor(data$Rebsorte)
data$Geschmack = as.factor(data$Geschmack)
data$Weinstil = as.factor(data$Weinstil)
data$Qualitätsstufe = as.factor(data$Qualitätsstufe)
data$Trinkreife = as.factor(data$Trinkreife)
data$Ausbau = as.factor(data$Ausbau)
data$Hersteller =as.factor(data$Hersteller)
data$Trinktemperatur = as.numeric(data$Trinktemperatur)
data$Restsüße = as.numeric(data$Restsüße)
data$Säuregehalt = as.numeric(data$Säuregehalt)
data$Jahrgang = as.integer(data$Jahrgang)
data$Preis = as.numeric(data$Preis)

save(file = "weinfreunde_data_1.RData", list="data")
